<div>
    <x-page title="{{ $this->isEditing() ? 'Kundenleistung bearbeiten' : 'Kundenleistung anlegen' }}"
            subtitle="{{ $customer->customer_number }} · {{ $customer->displayName() }}"
            back-label="Leistungen ／ zurück"
            back-url="{{ $this->isEditing() ? route('customer-services.show', [$customer, $service]) : route('customers.show', ['customer' => $customer, 'bereich' => 'leistungen']) }}">
        ...
        <form wire:submit="save" class="space-y-6">
            <x-card>
                <x-slot:header>
                    <div>
                        <h2 class="text-sm font-semibold text-ink">Herkunft</h2>
            ...
            <x-card>
                <x-slot:header>
                    <h2 class="text-sm font-semibold text-ink">Preise und Abrechnung</h2>
                </x-slot:header>
