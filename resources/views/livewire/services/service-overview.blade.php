<div>
    <div class="mb-6">
        <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Leistungsübersicht</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Alle Kundenleistungen mit vereinbartem Preis, Kosten und Abrechnungsintervall.
        </p>
    </div>
    ...
    <x-card>
        ...
        <x-table :headers="$this->tableHeaders()" :rows="$services" :sort="$sort" paginate>
            ...
            @interact('column_product', $row)
                ...
                @else
                    <span class="text-gray-400">Individuell</span>
                @endif
            @endinteract

            @interact('column_category', $row)
                @if ($row->category && $row->subcategory)
                    {{ $row->category->name }} <span class="text-gray-400">&rarr;</span> {{ $row->subcategory->name }}
                @else
                    {{ $row->category?->name ?? '—' }}
                @endif
            @endinteract
            ...
            @interact('column_margin', $row)
                <span class="tabular-nums {{ $row->margin()->isNegative() ? 'text-red-600 dark:text-red-400' : '' }}">
                    {{ $row->margin()->format() }}
                    @if ($row->marginPercentage() !== null)
                        <span class="text-gray-400">({{ number_format($row->marginPercentage(), 1, ',', '.') }} %)</span>
                    @endif
                </span>
            @endinteract
