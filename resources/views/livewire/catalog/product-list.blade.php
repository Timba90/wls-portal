<div>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Artikel / Leistungen</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Der zentrale Katalog mit Standardpreisen und Abrechnungsintervallen.
            </p>
        </div>
    ...
    <x-card>
        ...
        <x-table :headers="$this->tableHeaders()" :rows="$products" :sort="$sort" paginate>
            ...
            @interact('column_margin', $row)
                <span class="tabular-nums {{ $row->defaultMargin()->isNegative() ? 'text-red-600 dark:text-red-400' : '' }}">
                    {{ $row->defaultMargin()->format() }}
                    @if ($row->defaultMarginPercentage() !== null)
                        <span class="text-gray-400">({{ number_format($row->defaultMarginPercentage(), 1, ',', '.') }} %)</span>
                    @endif
                </span>
            @endinteract
