...
| Browser-Tests | Der Rundgang in `tests/Browser/SmokeTest.php` ist geschrieben, `pestphp/pest-plugin-browser` und `playwright` sind eingerichtet. Der Lauf bleibt nach dem Start des Playwright-Servers hängen, statt einen Browser zu öffnen; die Suite ist deshalb in `phpunit.xml` nicht eingetragen, damit sie die CI nicht blockiert. Zwischenstand der Ursachensuche steht im PR. |
