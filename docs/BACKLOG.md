...
| Element | Umsetzung |
|---|---|
| Täglicher Abgleich (§60) | `registrar:sync` läuft nachts um 03:20 über alle eingerichteten Anbieter. Nachts, weil niemand darauf wartet; `withoutOverlapping()` und `onOneServer()`, damit sich zwei Läufe nicht in die Quere kommen. |
| Ein Fehlschlag darf nicht still sein | Das ist der eigentliche Punkt. Ein Abgleich, der nachts scheitert, sieht am nächsten Morgen aus wie ein Bestand ohne Änderungen — man merkt es erst, wenn eine Domain unbemerkt abgelaufen ist. Jeder Lauf trägt sich deshalb in `registrar_syncs` ein, mit Zahlen oder mit der Meldung des Anbieters im Wortlaut, und der letzte Lauf steht unter „Schnittstellen" beim jeweiligen Anbieter — im Fehlerfall in Rot. |
| Jeder Fehlschlag, nicht nur der erwartete | Zuerst fing der Abgleich nur `RegistrarException`. Ein Datenbankfehler mitten im Lauf wäre ein Eintrag ohne Ende und ohne Meldung gewesen — und hätte die übrigen Anbieter mitgerissen, also genau das verhindert, wofür das Protokoll da ist. Gefangen wird jetzt `Throwable`; bei einem unerwarteten Fehler steht die Ausnahme im Log-Kontext. |
| Planmäßig oder von Hand | Der Zeitplan ruft `registrar:sync --geplant` auf; ohne die Angabe gilt ein Lauf als von Hand angestoßen, denn dann hat ihn jemand getippt. Vorher stand jeder Lauf als „planmäßig" im Protokoll, und die Unterscheidung in der Anzeige war totes Blech. |
| Kein Wiederholen | Bei ResellerInterface verlängert jeder weitere Versuch eine Sperre, und der nächste planmäßige Lauf ist früh genug. Ein Fehlschlag bei einem Anbieter hält die übrigen nicht auf; der Rückgabewert meldet ihn trotzdem, damit der Zeitplan ihn sieht. |
| Der Zeitplaneintrag hat einen eigenen Test | Der ganze Sinn ist, dass er ohne Zutun läuft. Fällt der Eintrag heraus, fiele das sonst erst auf, wenn der Bestand veraltet ist. |
| `registrar_syncs` ist nicht auditierbar | Die Änderungshistorie hält fest, was Menschen an Daten tun. Ein Abgleich ist ein Betriebsvorgang und führt sein eigenes Protokoll. |
...
| Zuordnung von Hand | Die andere Hälfte des Imports. Der Registrar liefert den technischen Bestand, wem er gehört weiß nur das Portal — ohne diesen Schritt sind die eingelesenen Domains Daten zum Ansehen, nicht zum Arbeiten. Ein Knopf je Zeile öffnet das Formular; die Kundenleistung darüber ist die Verbindung zur Abrechnung und bleibt freiwillig, weil nicht jede Domain einzeln berechnet wird. Die Logik steht in `App\Actions\Registrar\AssignInventory`, die Oberfläche in einem Concern, den sich beide Listen teilen. |
| Kunde wechseln räumt die Leistung ab | Sowohl in der Oberfläche (beim Umstellen des Kunden) als auch in der Aktion (eine Leistung ohne Kunde und eine Leistung fremder Herkunft werden abgewiesen). Sonst hinge die Leistung des alten Kunden am neuen — und die Abrechnung zeigte auf die falsche Stelle. |
| Filter „Ohne Leistung" | Auf Ansage. „Ohne Kunde" und „Ohne Leistung" sind zwei verschiedene Lücken: dort fehlt die Zuordnung ganz, hier steht der Kunde fest und nur die Verbindung zur Abrechnung fehlt — und genau die entscheidet, ob eine Domain im Soll-Umsatz auftaucht. Als Filter und als eigene Kennzahlkachel in beiden Listen. |
| Zuordnen über MCP | `bestand-suchen` und `bestand-zuordnen`. Dieselbe Aktion wie in der Oberfläche, also dieselben Regeln: eine Leistung ohne Kunde und eine Leistung fremder Herkunft werden abgewiesen, und ohne Kunde fällt die Leistung mit weg. Der Suchfilter kennt `ohne_kunde`, `ohne_leistung` und `zugeordnet`; die Server-Anleitung erklärt den Unterschied, weil er sich aus den Namen allein nicht erschließt. |
...
| ResellerInterface über die Brücke, nicht direkt | Das Portal läuft auf demselben Server wie die Brücke (`domain-api-call`) und ruft sie lokal auf — kein SSH-Umweg, aber auch **kein eigener Login**. Die Zugangsdaten bleiben in der Umgebung der Brücke, die Anmeldung und die Sitzung macht sie. Grund ist ein echter Vorfall: ein selbstgebautes Login-Skript hat sich mit leeren Zugangsdaten in Schleife angemeldet, das Konto wurde gesperrt und die DNS-Änderungen aller Kunden waren blockiert. |
| Drei Regeln als Code, nicht als Kommentar | (1) Eine Positivliste erlaubter Aktionen — nur `domain/list` und `domain/check`; was nicht draufsteht, erreicht die Brücke gar nicht. (2) Kein Wiederholen: ein fehlgeschlagener Aufruf wird nie ein zweites Mal versucht, weil jeder Versuch eine Sperre verlängert. (3) `TOO_MANY_ATTEMPTS` und `WRONG_USERNAME_OR_PASSWORD` brechen mit einer eigenen, unmissverständlichen Meldung ab. Jede Regel hat ihren Test. |
| `limit: 1000` bei `domain/list` | Die Voreinstellung des Anbieters ist 25. Ohne das Limit fehlen mehrere hundert Domains, und der Bestand sieht aus, als wäre er verschwunden. Subreseller kommen nur mit `resellerID` mit — konfigurierbar als Liste. |
| Keine Zugangsdaten im Portal für ResellerInterface | Der Schnittstellen-Bildschirm zeigt für diesen Anbieter kein einziges Feld, sondern den Hinweis, wo die Anmeldung stattfindet. Ob er eingerichtet ist, beantwortet der Anschluss selbst (ist die Brücke ausführbar?) statt die Oberfläche über hinterlegte Felder. |
| Zertifikate bei ResellerInterface | Der Anschluss führt keine. Die Anleitung nennt `domain/*`, `dns/*` und `handle/*`; ein Zertifikatsbestand kommt nicht vor. Eine leere Liste ist die ehrliche Antwort — geraten wird nichts. |
| Feldnamen von ResellerInterface | Noch **nicht** gegen eine echte Antwort belegt: die Umgebung hier erreicht die Brücke nicht, und die vollständige Referenz liegt auf wls-lab. Der Anschluss liest deshalb die naheliegenden Schreibweisen und bricht mit der Rohantwort ab, wenn keine passt; `data.list` als Ort der Liste ist aus der Anleitung belegt. Erster Schritt am Zielsystem: `php artisan registrar:test resellerinterface`, dann `registrar:import resellerinterface --trocken`. |
| Anbieter | autoDNS (InterNetX Domainrobot), `https://api.autodns.com/v1/`. Die zuvor gebauten Anschlüsse an INWX und ein geratenes Reseller-Portal sind entfernt — der Auftraggeber hat autoDNS benannt, alles andere wäre Vorrat gewesen. Angemeldet wird mit Basic-Auth und dem Kontext im Kopf `X-Domainrobot-Context`. Unser Kontext ist **4**; er ist kein Geheimnis und steht deshalb voreingestellt in `config/services.php` statt bei den Zugangsdaten. Einzutippen sind nur Benutzername und Kennwort. Das Feld bleibt trotzdem stehen — fürs Testsystem von autoDNS gilt 1 —, ein hinterlegter Wert geht vor. |
| Feldnamen der Schnittstelle | Aus der offiziellen OpenAPI-Beschreibung des Anbieters übernommen (`InterNetX/domainrobot-api`, `src/domainrobot.json`), nicht geraten: `name`/`idn`, `registryStatus`, `domainCreated`, `expire`, `autoRenewStatus`, `nameServers[].name`; bei Zertifikaten `id`, `product`, `subjectAlternativeNames[].name`. Beim vorherigen Anlauf hatte Raten schon einen echten Fehler erzeugt. Gegen ein echtes Konto ist noch nichts gelaufen; ohne verwertbare Antwort bricht der Anschluss mit der Rohantwort ab, statt leere Datensätze anzulegen. |
| Verbindungstest vor dem Import | Auf Ansage. autoDNS bietet `/hello` — der Aufruf liest nichts, ändert nichts und beantwortet genau die Frage, ob Zugangsdaten und Kontext stimmen. Erreichbar über „Verbindung prüfen" unter „Schnittstellen" und über `php artisan registrar:test`. Die Antwort des Anbieters wird im Wortlaut gezeigt: bei einer Ablehnung ist genau sie der Hinweis, was fehlt. Danach erst `php artisan registrar:import --trocken`. |
| `autoRenewStatus` | autoDNS kennt `TRUE`, `FALSE` und `ONCE`. Nur `TRUE` verlängert dauerhaft; `ONCE` einmal. Als `auto_renew` gilt deshalb allein `TRUE` — ein Häkchen bei `ONCE` verspräche eine Verlängerung, die nicht kommt. |
| Domains ohne eigene Kennung | autoDNS vergibt je Domain keine Kennung; der Name ist dort der Schlüssel. `provider_reference` bleibt für Domains deshalb leer, der Abgleich läuft über den Namen. Bei Zertifikaten gibt es die `id`, und ohne sie wird übergangen. |
| Zugangsdaten | Verschlüsselt in der Datenbank (§50), gepflegt unter „Schnittstellen". Die Felder sind einseitig: was hinterlegt ist, wird nie zurückgelesen — ein Kennwort, das die Seite wieder ausliefert, steht im Quelltext des Browsers. Angezeigt wird nur, ob etwas hinterlegt ist. In der Umgebung steht nur noch ein abweichender Endpunkt, etwa für ein Testsystem. |
| Offene Punkte aus §60 | Nicht in diesem Schritt: **do.de / ResellerInterface** als Anbieter (§60 nennt ihn als Hauptprovider), **manuelle Pflege** von Domains anderer Anbieter, **täglicher Sync**. Schreibende Vorgänge — Registrierung, Transfer, DNS, Verlängerung, Kündigung — sind bewusst nicht enthalten; §60 plant sie mit Bestätigung für kritische Aktionen. |
| Notizen, Dokumente, eigene Felder an Domains | §525 und §933 sehen sie vor, hier noch nicht angebunden. Die Concerns dafür gibt es bereits (`HasNotes`, `HasDocuments`, `HasCustomFields`); das Nachziehen ist eine Zeile je Modell plus Oberfläche. |
| Nur ein Anbieter | `RegistrarProvider` hat noch genau einen Fall. Die Trennung in Schnittstelle, Anschluss und Fabrik bleibt trotzdem: do.de kommt laut §60 dazu, und der Import kennt den Anbieter nur als `RegistrarClient`. |
...
| Sortierrichtung aus der URL | Vier Listen prüften die Sortierspalte, aber nicht die Richtung. Beides steht in einer öffentlichen Eigenschaft und ist damit vom Browser setzbar; bei einem fremden Wert wirft Laravel eine `InvalidArgumentException`, und die Seite antwortete mit 500 statt mit einer Liste. Eingeschleust werden konnte nichts — Laravel lässt nur `asc` und `desc` durch —, aber eine gebastelte Adresse legte die Seite lahm. Ein Datensatz-Test über alle sieben Listen hält beides fest. |
